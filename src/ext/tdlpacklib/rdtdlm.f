      SUBROUTINE RDTDLM(KFILDO,KFILX,CFILX,ID,RECORD,NSIZE,NVALUE,
     1                  L3264B,IER) 
C 
...
C                                IRAEND
C        OCTOBER  2012   ENGLE   ADDED NARAEND( ) TO COMMON BLOCK ARGC.
C 
C        PURPOSE 
C            TO READ A RECORD TO FROM MOS-2000 EXTERNAL DIRECT ACCESS
...
C                       (INPUT)
C           RECORD(J) = THE ARRAY TO READ DATA INTO (J=1,NSIZE).
C                       (INPUT)
...
C
      CALL RDTM(KFILDO,KFILX,ID,RECORD,NSIZE,NVALUE,
