...
C      IB=(IS4(2).AND.IB10)
C      IF(IB.NE.0)GO TO 3035
      IF(BTEST(IS4(2),3))GO TO 3035
...
C
 3035 MISSP=0
C      IB=IS4(2).AND.2
C      IF(IB.EQ.0)GO TO 304
      IF(.NOT.BTEST(IS4(2),1))GO TO 304
...
C      IB=IS4(2).AND.1
C      IF(IB.EQ.0)GO TO 304
      IF(.NOT.BTEST(IS4(2),0))GO TO 304
...
C        IN THE PACKING AND UNPACKING PROCESSES.
 ...
 304  CONTINUE
C
CD    WRITE(KFILDO,3040)(IS0(K),K=1,3)
CD3040 FORMAT(/,' IS0',2X,A4,2I10)
CD    WRITE(KFILDO,3041)(IS1(K),K=1,22+IS1(22))
CD3041 FORMAT(/,' IS1',10I11,/,'    ',10I11,/,'    ',2I11,4X,32R1)
CD    IF(IS2(1).NE.0)WRITE(KFILDO,3042)(IS2(K),K=1,12)
C        WHEN THE DATA ARE VECTOR, IS2( ) WILL NOT BE PRESENT.
CD3042 FORMAT(/,' IS2',10I11,/,('    ',10I11))
CD    WRITE(KFILDO,3043)(IS4(K),K=1,5)
CD3043 FORMAT(/,' IS4',10I11,/,('    ',10I11))
C
...
C      IF(IB.EQ.0)GO TO 310
      IF(.NOT.BTEST(IS4(2),2))GO TO 310
C
...
C
      ...
      IF(IER.NE.0)GO TO 900
C
CD    WRITE(KFILDO,331)LX
CD331  FORMAT(/,' ',I7,' GROUPS')
CD    WRITE(KFILDO,332)(JMIN(K),K=1,LX)
CD332  FORMAT(/,' JMIN( ) =',/,(' ',15I6))
CD    WRITE(KFILDO,333)(LBIT(K),K=1,LX)
CD333  FORMAT(/,' LBIT( ) =',/,(' ',15I6))
CD    WRITE(KFILDO,334)(NOV(K),K=1,LX)
CD334  FORMAT(/,' NOV( ) =',/,(' ',15I6))
C
...
C
C 380  IB=IS4(2).AND.4
C      IF(IB.EQ.0)GO TO 391
 380  IF(.NOT.BTEST(IS4(2),2))GO TO 391
C
...
C
C      IB=IS4(2).AND.IB20
C      IF(IB.NE.0)GO TO 400
      IF(.NOT.BTEST(IS4(2),4))GO TO 400
C
