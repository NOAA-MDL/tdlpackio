...
def get_package_info(name, static=False, required=True, include_file=None):
    ...
    if pkg_dir is not None:
        ...
        for libname in libnames:
            libpath = find_library(libname, static=static, required=required)
    ...
    return libname_to_use, pkg_incdir, pkg_libdir
...
def run_ar_command(filename):
    """Run the ar command"""
    cmd = subprocess.run(["ar", "-t", filename], stdout=subprocess.PIPE)
...
def run_ldd_command(filename):
    """Run the ldd command"""
    cmd = subprocess.run(["ldd", filename], stdout=subprocess.PIPE)
    cmdout = cmd.stdout.decode("utf-8")
    return cmdout
...
def run_otool_command(filename):
    ...
    cmd = subprocess.run(["otool", "-L", filename], stdout=subprocess.PIPE)
    cmdout = cmd.stdout.decode("utf-8")
    return cmdout
...
extmod_config["tdlpacklib"] = dict(libraries=[pkginfo[0]], incdirs=[pkginfo[1]], libdirs=[pkginfo[2]], extra_objects=[])
...
# Define extensions
# ----------------------------------------------------------------------------------------
tdlpacklibext = Extension(
    "tdlpackio.tdlpacklib",
    [tdlpacklib_pyx],
...
# ----------------------------------------------------------------------------------------
cnt = """# This file is generated by tdlpackio's setup.py
